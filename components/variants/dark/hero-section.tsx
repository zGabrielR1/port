...
export function HeroSection() {
  ...
  return (
    <section className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-black/35 via-transparent to-black/15" />
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 text-center relative z-10">
        <div className="space-y-12">
          <div className="space-y-8">
            ...
            {timeMode === 'clock' ? (
              <div className={`glassmorphism px-8 py-12 rounded-3xl shadow-xl transition-all duration-700 ${
                visible ? "opacity-100 scale-100" : "opacity-0 scale-95"
              }`}>
                <TimeDisplay showLocation showTemperature timeFormat="12h" />
                <div className="mt-8 text-white/70 text-sm">Welcome to my digital space</div>
              </div>
            ) : (
              <div className={`glassmorphism px-8 py-10 rounded-3xl shadow-xl transition-all duration-700 ${
                visible ? "opacity-100 scale-100" : "opacity-0 scale-95"
              }`}>
                <h1 className="text-5xl sm:text-6xl lg:text-8xl font-bold text-white leading-tight tracking-tight mb-4">
                  Hi, I'm <span className="bg-gradient-to-r from-blue-400 via-purple-400 to-blue-400 bg-clip-text text-transparent animate-gradient-x">Gabriel Renostro</span>
                </h1>
                <p className="text-xl sm:text-2xl lg:text-3xl text-white/90 max-w-3xl mx-auto font-medium leading-relaxed mb-4">Full-Stack Developer & Software Engineer</p>
                <p className="text-base sm:text-lg text-white/80 max-w-2xl mx-auto leading-relaxed">I create exceptional digital experiences with modern technologies, focusing on clean code and user-centered design that makes a difference.</p>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col items-center gap-8">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8">
              <GlassmorphismButton onClick={scrollToAbout} variant="primary" size="lg" className="px-8 py-6 text-lg font-medium group" showShine showRipple>
                <span>Explore My Work</span>
                <ArrowDown className="ml-2 h-5 w-5 group-hover:translate-y-1 transition-transform duration-300" />
              </GlassmorphismButton>

              <GlassmorphismButton variant="default" size="lg" className="px-8 py-6 text-lg font-medium group" showShine showRipple>
            ...
            <div className="flex items-center gap-2 p-4 rounded-xl glassmorphism animate-glassmorphism-pulse">
              ...
              <div className="flex items-center gap-2">
                <GlassmorphismButton variant="default" size="sm" className="h-10 w-10 p-0" aria-label="GitHub Profile" showShine={false}><Github className="h-4 w-4" /></GlassmorphismButton>
                <GlassmorphismButton variant="default" size="sm" className="h-10 w-10 p-0" aria-label="LinkedIn Profile" showShine={false}><Linkedin className="h-4 w-4" /></GlassmorphismButton>
                <GlassmorphismButton variant="default" size="sm" className="h-10 w-10 p-0" aria-label="Email Contact" showShine={false}><Mail className="h-4 w-4" /></GlassmorphismButton>
              </div>
            </div>
          </div>
